// ============================================================
// POSTS ROUTES â€” CRUD para sa Posts/Blogs/Articles
// ðŸ‘‰ BASE URL: /api/posts
// ðŸ‘‰ PALITAN ANG "Post" NG IYONG MODEL (Blog, Article, News, etc.)
// ðŸ‘‰ PALITAN ANG MGA FIELD (title, content, etc.) NG IYONG MGA FIELD
// ============================================================
...
// -------------------------------------------------------
// GET /api/posts â€” Lahat ng posts (public)
// -------------------------------------------------------
...
// -------------------------------------------------------
// GET /api/posts/:id â€” Isang post
// -------------------------------------------------------
router.get("/:id", verifyToken, (req, res) => {
  try {
    // ðŸ‘‰ PALITAN: const post = await Post.findById(req.params.id)
...
// -------------------------------------------------------
// POST /api/posts â€” Gumawa ng bagong post (kailangan ng login)
// -------------------------------------------------------
router.post("/", verifyToken, (req, res) => {
  try {
    // ðŸ‘‰ PALITAN ANG MGA FIELD NA ITO
    ...
    // ðŸ‘‰ PALITAN: const post = await Post.create({...})
    const newPost = {
...
// -------------------------------------------------------
// PUT /api/posts/:id â€” I-edit ang post (author lang)
// -------------------------------------------------------
...
// -------------------------------------------------------
// POST /api/posts/:id/like â€” I-like ang post
// -------------------------------------------------------
router.post("/:id/like", verifyToken, (req, res) => {
  try {
    // ðŸ‘‰ PALITAN: await Post.findByIdAndUpdate(req.params.id, { $inc: { likes: 1 } })
    const post = posts.find((p) => p.id === req.params.id);
    if (!post) return errorResponse(res, "Hindi mahanap ang post.", 404);
...
// -------------------------------------------------------
// DELETE /api/posts/:id â€” Burahin ang post (author o admin)
// -------------------------------------------------------
router.delete("/:id", verifyToken, (req, res) => {
  try {
    ...
    // ðŸ‘‰ PALITAN: await Post.findByIdAndDelete(req.params.id)
